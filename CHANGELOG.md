...
- New behavior for api module => an api module in an undefined workspace will be deleted on vscode closing.
While in a defined workspace, the module will persists.
...
- Feature to change Simplicite resource files such as _.css_, _.less_, _.js_, _.html_, _.md_, _.xml_, _.xml_, _.txt_, _.yaml_.
- File icons in the file tree view.

### Removed

- Feature to track/untrack a file only with its file name, due to the implementation of the resource files that implies
  that some files have the same name.
